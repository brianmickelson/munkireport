...
cd ${MUNKIPATH}
curl --fail --silent \
	"${TPL_BASE}{preflight,postflight,report_broken_client}"\
	--remote-name --remote-name --remote-name \
&& curl --fail --silent "${TPL_BASE}reportcommon" -o \
...
curl --fail --silent "${TPL_BASE}{submit.preflight,disk_info}" \
...
# Add munkireport
defaults write "${PREFPATH}" ReportItems -dict-add Munkireport \
...
# Add inventory
defaults write "${PREFPATH}" ReportItems -dict-add InventoryItem \
	"/Library/Managed Installs/ApplicationInventory.plist"

# Add disk_info
defaults write "${PREFPATH}" ReportItems -dict-add DiskReport \
	"${MUNKIPATH}preflight.d/cache/disk.plist"
